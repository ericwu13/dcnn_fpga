...
//            }
            for(result_c=0 ; result_c<256 ; ++result_c){
               if(db*256+result_c<flatten_num)
                  *(output_mat+(db*256+result_c)*filter_num + i) += (result[result_c]-counter);
            }
         }
      }
   }
...
//            }
            for(result_c=0 ; result_c<256 ; ++result_c){
               if(db*256+result_c<flatten_num)
                  *(fc_finish+(db*256+result_c)*filter_num + i) += (result[result_c] - counter);
            }
         }
      }
   }
...
//        	if(row == 8) {
//        		if(column > 1) matrix[row][column] = -43;
//        		else matrix[row][column] = 32;//-row+column;
//        	}
//        	else {
//        		if(row > 4 || column > 5) matrix[row][column] = -122;
//        		else matrix[row][column] = 11;
//        	}
//        }
//     }
//     for(row = 0; row < 2; row ++){
...
//     return 0;
  ...
  alt_u8 filter[15][15][1][1] = {0};
  int row = 0;
  int column = 0;
  printf("ha ha ..........................\n");
  for(row = 0; row < 20; row ++){
	for(column = 0; column < 20; column++){
		matrix[row][column] = 30;
	}
  }
  for(row = 0; row < 15; row ++){
	for(column = 0; column < 15; column++){
		filter[row][column][0][0] = 1;//+row+column;
		filter[row][column][0][0] = 7;//+row+column;
	}
  }
  alt_u8 flatten [36][225] = {0};
  alt_16 output_mat[36][1] = {0};
  alt_u8  con2_fmap[6][6][1] = {0};
  flatten_matrix(flatten, matrix, 20, 20, 15, 15, 1);
  //printMatrix(flatten, 121, 100, 1, 16);
  conv(flatten, 6, 6, filter, 15, 15, 1, con2_fmap, output_mat);
  printMatrix(con2_fmap, 6, 6, 1, 16);
  return 0;
